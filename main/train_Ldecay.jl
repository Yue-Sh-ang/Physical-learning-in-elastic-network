...
if !isdir(task_path)
    mkdir(task_path)
    generate_task(net,task_path; s_in=[s_target],s_out=[s_target],Distant=true,seed=taskid)
end
...
# training loop
trainpath=joinpath(task_path, "trainT$(trainT)_alpha$(alpha_0)_tw$(timewindow)_Ldecay","seed$(seed)")
rng=StableRNG(seed)
mkpath(trainpath)
t0=time()
...
for stepid in 1:trainsteps
    ...
    for sid in 1:timewindow 
        ...
        strain_f_out=cal_strain(train0.net_f,train0.output[1][1],l0=train0.output[1][3])
        error+=abs(strain_f_out - s_target)/timewindow
    end
        
    update_k!(train0, Gradient, alpha_0/(s_target/2.0)*error) # linear function and will cross (target/2,alpha_0)
